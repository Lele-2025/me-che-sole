import { C, brandPattern } from "../theme.js";
  ...
  return (
    <div onClick={()=>onDettaglio(l)} className="card-hover"
      ...
      <div style={{height:88,background:C.sabbia,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2.6rem",position:"relative"}}>
        {l.img}
        {l.stelle===5&&<span style={{position:"absolute",top:8,right:8,background:C.white,border:"1px solid "+C.line,borderRadius:50,padding:"2px 9px",fontSize:"0.62rem",fontWeight:800,color:C.brandOrange}}>★ TOP</span>}
      ...
      {/* firma "onda" del brand, dettaglio sottile — non sfondo pieno */}
      <div style={{height:5,backgroundImage:brandPattern,backgroundSize:"40px 40px"}}/>
      <div style={{padding:"1rem"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontFamily:"'DM Mono',monospace",fontSize:"1.15rem",fontWeight:500,color:C.brandNavy}}>da €{l.prezzoMin}</div>
            <div style={{fontSize:"0.62rem",color:C.inkMuted}}>al giorno</div>
          </div>
          <div style={{background:C.brandNavy,color:C.white,borderRadius:10,padding:"0.55rem 1.1rem",fontSize:"0.8rem",fontWeight:700}}>Prenota →</div>
        </div>
      </div>
    </div>
  );
