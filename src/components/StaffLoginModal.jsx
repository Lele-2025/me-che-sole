...
// ─── LOGIN STAFF (gestore o admin) ───────────────────────────────────
// Un solo form di accesso via Firebase Auth. Il ruolo (gestore di uno
// specifico lido, o admin) è deciso dai custom claim sul token, non
// da questo componente: vedi handleStaffLogin in App.jsx.
export function StaffLoginModal({onLogin,onClose,err,loading}) {
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,58,88,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:"1rem",backdropFilter:"blur(4px)"}}>
      <div style={{background:"#002A40",borderRadius:20,padding:"2rem",width:"100%",maxWidth:380,border:"2px solid rgba(41,182,206,0.4)",boxShadow:"0 24px 60px rgba(0,0,0,0.5)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.5rem"}}>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.4rem",fontWeight:700}}><span style={{color:"#68D8C8"}}>Accesso</span> <span style={{color:"#F4C430"}}>riservato</span> 🔑</div>
        ...
        <input style={inp} type="email" placeholder="nome@esempio.it" value={email} onChange={e=>setEmail(e.target.value)} autoComplete="username"/>
        ...
        <input style={inp} type="password" placeholder="••••••••" value={pass} onChange={e=>setPass(e.target.value)} autoComplete="current-password" onKeyDown={e=>e.key==="Enter"&&!loading&&onLogin(email,pass)}/>
